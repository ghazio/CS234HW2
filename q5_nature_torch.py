...
class NatureQN(Linear):
    ...
    def initialize_models(self):
        ...
        self.q_network = nn.Sequential(nn.Conv2d(channels,16,3),nn.ReLU(),nn.Flatten(),nn.Linear((img_width-2)*(img_height-2)*16,128),nn.ReLU(),nn.Linear(128,num_actions))
        self.target_network = nn.Sequential(nn.Conv2d(channels,16,3),nn.Flatten(),nn.ReLU(),nn.Linear((img_width-2)*(img_height-2)*16,128),nn.ReLU(),nn.Linear(128,num_actions))
        ##############################################################
        ######################## END YOUR CODE #######################
