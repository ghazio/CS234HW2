...
class Linear(DQN):
    ...
    def get_q_values(self, state: torch.Tensor, network: str = "q_network"):
        ...
        out = out.reshape(batch_size,self.env.num_actions())
        ##############################################################
        ######################## END YOUR CODE #######################
        return out
    ...
    def update_target(self):
        ...
        #get the weights of the q network, and load them on the target network
    ...
    def calc_loss(
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        q_samp = torch.zeros(q_values.shape[0])
        q_val_sa = torch.zeros(q_values.shape[0])
        #pdb.set_trace()
    
        for i,done in enumerate(done_mask):
            #print(i)
            #pdb.set_trace()
            #save the action actually taken
            q_val_sa[i] = q_values[i][actions[i].long()]
            if done:
                #print("reward",rewards[i])
                q_samp[i] = rewards[i]
            else:
                #print("reward",rewards[i]+gamma * torch.max(target_q_values[i]))
                q_samp[i] = rewards[i] + gamma * torch.max(target_q_values[i])
                
        #pdb.set_trace()
        loss_function = torch.nn.MSELoss()
        loss = loss_function(q_samp,q_val_sa)
